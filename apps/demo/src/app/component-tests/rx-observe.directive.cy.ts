  ...
  return {
    render: () => {
      cy.mount(TestHostComponent, {
        imports: [DirtyCheckerComponent, RxObserveDirective, TestHostComponent],
        providers: [
          cfg?.streamConfig
            ? provideRxObserveDirectiveConfig(cfg?.streamConfig)
            : [],
        ],
      });
    },
...
@Component({
  ...
  template: `
    ...
    <div>
      ...
      >
        <span
          >Value: <span id="value">{{ value | json }}</span>
        </span>
        RenderCount:
  ...
  standalone: true,
  imports: [DirtyCheckerComponent, CommonModule, RxObserveDirective],
})
